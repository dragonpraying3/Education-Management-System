...
def schedule():
    ...
    for teacher in teachers:
        print(
            f"{counter}. Teacher ID: {teacher['Teacher ID']} - Instructor: {teacher['Instructor']} - Available Time: {teacher['Available time']}")  # List teachers
        counter += 1
    ...
    if new_time:
        ...
        with open("teachers.txt", "w") as f:
            ...
            for teacher in teachers:
                f.write(
                    f"{teacher['Teacher ID']},{teacher['Day']},{teacher['Instructor']},{teacher['Available time']}\n")
    else:
